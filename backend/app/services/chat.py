...
class ChatService:
    ...
    def _get_llm(self, model_name: Optional[str] = None, streaming: bool = False) -> ChatOpenAI:
    ...
    async def process_chat(
        ...
        try:
            ...
            llm = self._get_llm(chat_request.model)
            tools = [online_search, get_weather, get_system_time]
    ...
    async def handle_rag_chat(
        ...
        ai_msg_db = await chat_crud.create_message(
            ...
            content=ai_content,
            kb_id=req.kb_id,
            model_name=req.model,  # 保存使用的模型名称
    ...
    async def stream_chat(
        ...
        # 进入标准流式 LLM 输出（天气问题也走 agent）
        llm = self._get_llm(chat_request.model, streaming=True)
        full_content = ""
    ...
    async def stream_rag_chat(
        ...
        llm = self._get_llm(req.model, streaming=True)
        full_content = ""
